 ...
 */
public class IdleStateHandlerEx extends ChannelDuplexHandler {
	 ...
	 */
	public IdleStateHandlerEx(int readerIdleTimeSeconds,
	 ...
	 */
	public IdleStateHandlerEx(long readerIdleTime, long writerIdleTime,
	...
	/** 调整心跳监测时间 **/
	public void setReaderIdleTimeMillis(long readerIdleTimeMillis) {
		this.readerIdleTimeMillis = Math.max(readerIdleTimeMillis, 1);
	}
	...
	private void initialize(ChannelHandlerContext ctx) {
		...
		if (readerIdleTimeMillis > 0) {
			readerIdleTimeout = loop.schedule(new ReaderIdleTimeoutTask(ctx),
					readerIdleTimeMillis, TimeUnit.MILLISECONDS);
		}
		if (writerIdleTimeMillis > 0) {
			writerIdleTimeout = loop.schedule(new WriterIdleTimeoutTask(ctx),
					writerIdleTimeMillis, TimeUnit.MILLISECONDS);
		}
		if (allIdleTimeMillis > 0) {
			allIdleTimeout = loop.schedule(new AllIdleTimeoutTask(ctx),
	...
	private final class ReaderIdleTimeoutTask implements Runnable {
		...
		public void run() {
			...
			long lastReadTime = IdleStateHandlerEx.this.lastReadTime;
			long nextDelay = readerIdleTimeMillis
					- (currentTime - lastReadTime);
			if (nextDelay <= 0) {
				// Reader is idle - set a new timeout and notify the callback.
				readerIdleTimeout = ctx.executor().schedule(this,
						readerIdleTimeMillis, TimeUnit.MILLISECONDS);
				try {
				...
				// shorter delay.
				readerIdleTimeout = ctx.executor().schedule(this, nextDelay,
	...
	private final class WriterIdleTimeoutTask implements Runnable {
		...
		public void run() {
			...
			long currentTime = System.currentTimeMillis();
			long lastWriteTime = IdleStateHandlerEx.this.lastWriteTime;
			long nextDelay = writerIdleTimeMillis
					- (currentTime - lastWriteTime);
			if (nextDelay <= 0) {
				// Writer is idle - set a new timeout and notify the callback.
				writerIdleTimeout = ctx.executor().schedule(this,
						writerIdleTimeMillis, TimeUnit.MILLISECONDS);
				try {
					IdleStateEvent event;
				...
				// shorter delay.
				writerIdleTimeout = ctx.executor().schedule(this, nextDelay,
						TimeUnit.MILLISECONDS);
			}
		}
	}
	...
	private final class AllIdleTimeoutTask implements Runnable {
		...
		public void run() {
			...
			if (nextDelay <= 0) {
				...
				// notify the callback.
				allIdleTimeout = ctx.executor().schedule(this,
						allIdleTimeMillis, TimeUnit.MILLISECONDS);
				try {
					IdleStateEvent event;
				...
				allIdleTimeout = ctx.executor().schedule(this, nextDelay,
						TimeUnit.MILLISECONDS);
			}
		}
	}

--
module("UI.Windows.CreateRoomWin", package.seeall)
...
CreateRoomWin = UIWindow:new()

CreateRoomWin.name = "CreateRoomWin" --窗体名字
local type = {}

--初始化界面
function CreateRoomWin:Init()
    --初始化界面，保存引用控件
    ...
    self.TR_KaiFang  = UIUtility.GetChildTransform("TR_KaiFang",t,true)
    ...
    --固定庄
    local x = UnityEngine.PlayerPrefs.GetString("Game2Type", "")
    if x == "" then
        type.round = 2
        type.pType = 2
        type.bankerMode = 2
...
function CreateRoomWin:ColorToOn(transform)
    local wsxColorCom = UICreateRoomColor:new()
    wsxColorCom:Init(transform)
    wsxColorCom:On()
end

function CreateRoomWin:ColorToOff(transform)
    local wsxColorCom = UICreateRoomColor:new()
    wsxColorCom:Init(transform)
    wsxColorCom:Off()
end

function CreateRoomWin:ShowWin()
...
function CreateRoomWin:OnShow()
end

function CreateRoomWin:BindEvents()
    function PlayTypeCreater(playType, type)
        ...
        -- -- playTypeCreater = type
        local gameType = json.encode(type)
        UnityEngine.PlayerPrefs.SetString("Game2Type", gameType)
    end

    function CreateRoom()

        local RoomModule = require("Module.RoomModule").RoomModule
        RoomModule.CreateRoomBefore(type, PlayTypeCreater)
    end
    UIHelper.BindUIEvnet("Click", CreateRoom, self.crButton.gameObject)

-- 替人开房方法
    function CreateProxyRoom()

        Debug.log("替人开房")

        local RoomModule = require("Module.RoomModule").RoomModule
        RoomModule.CreateProxyRoomBefore(type, PlayTypeCreater)

        -- self:Close()
        
    end

    UIHelper.BindUIEvnet("Click", CreateProxyRoom, self.TR_KaiFang.gameObject)

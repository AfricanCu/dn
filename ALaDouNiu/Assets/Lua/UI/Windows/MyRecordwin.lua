--
module("UI.Windows.MyRecordWin",package.seeall)
...
MyRecordWin = UIWindow:new()
MyRecordWin.name = "MyRecordWin" 

function MyRecordWin:Init()
...
function MyRecordWin:BindEvents()
    ...
    UIHelper.BindUIEvnet("Click", FunClose, self.CloseBtn.gameObject)
end

function MyRecordWin:GameRecordSm(recordSm)
...
function MyRecordWin:ShowRecord()
    ...
    if self.Record and #self.Record>0 then
        for i=1, num do
            function OnAdd(obj)
                ...
                ChangShuLabel.text=i
                RoomNumLabel.text=self.Record[self.reCordLen+1-i].roomId.."号房间"

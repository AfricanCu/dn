 ...
 * restarts from Java. If _exitJvm ist set to true a hard System.exit() call is
 * being made.
 *
 ...
 * handlers.setHandlers(new Handler[] { someOtherHandler,
 * 		new ShutdownHandler(server, &quot;secret password&quot;) });
...
public class ShutdownHandler extends AbstractHandler {
	...
	public void handle(String target, Request baseRequest,
		...
		if (!hasCorrectSecurityToken(request)) {
			LOG.warn("Unauthorized shutdown attempt from "
	...
	private boolean requestFromLocalhost(HttpServletRequest request) {
		return getLocalIps().contains(getRemoteAddr(request));
	}
